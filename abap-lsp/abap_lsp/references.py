...
def _collect_references(node: Node, name: str, uri: str, locations: list[Location]):
    """Recursively collect all identifier/field_symbol nodes matching name (case-insensitive)."""
    if (
        node.type in ("identifier", "field_symbol")
        and node.text.decode().upper() == name.upper()
    ):
        locations.append(
            Location(
                uri=uri,
